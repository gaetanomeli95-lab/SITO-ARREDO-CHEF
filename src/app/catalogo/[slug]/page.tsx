...
export default function ProductPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <article className="relative overflow-hidden bg-[#e7e9ea] pb-20 pt-28 md:pb-28 md:pt-34 lg:pt-36">
        ...
        <div className="container-ac relative">
          <nav className="mb-6 flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-carbone/45 sm:mb-8">
          ...
          <div className="overflow-hidden rounded-[28px] border border-carbone/10 bg-white/75 shadow-[0_45px_110px_-55px_rgba(11,13,16,.48)] backdrop-blur-xl">
            <div className="grid lg:grid-cols-[1.05fr_.95fr]">
              <Reveal>
                <section className="relative min-h-[410px] overflow-hidden bg-[#111419] sm:min-h-[520px] lg:min-h-[720px]">
                  ...
                  <div className="absolute left-5 top-5 z-20 flex items-center gap-2 rounded-full border border-white/12 bg-black/25 px-3 py-2 text-[8px] font-bold uppercase tracking-[0.22em] text-white/72 backdrop-blur-md sm:left-7 sm:top-7">
                    <Radio size={10} className="text-rosso" /> Arredo Chef / Machine profile
                  </div>

                  <div className="absolute right-5 top-5 z-20 text-right text-[8px] font-bold uppercase leading-relaxed tracking-[0.22em] text-white/35 sm:right-7 sm:top-7">
                    {cp?.identity.model ? `MOD. ${cp.identity.model}` : 'Professional equipment'}
                  </div>

                  <div className="absolute inset-[16%_8%_11%] sm:inset-[14%_9%_10%] lg:inset-[15%_10%_10%]">
                    <Image
                      src={product.image}
                      alt={product.name}
                      fill
                      ...
                      className="object-contain drop-shadow-[0_45px_32px_rgba(0,0,0,.72)]"
                    />
                  </div>

                  <div className="absolute bottom-5 left-5 right-5 z-20 flex items-center justify-between gap-4 border-t border-white/12 pt-4 sm:bottom-7 sm:left-7 sm:right-7">
                    <span className="text-[9px] font-bold uppercase tracking-[0.22em] text-white/48">
                      {product.category}
                    </span>
              ...
              <section className="relative bg-[linear-gradient(145deg,rgba(255,255,255,.98),rgba(236,239,240,.96))] p-6 sm:p-8 lg:p-10 xl:p-12">
                <Reveal delay={0.08}>
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="inline-flex items-center rounded-full border border-rosso/25 bg-rosso/[0.06] px-3 py-1.5 text-[9px] font-bold uppercase tracking-[0.2em] text-rosso">
                      {cp ? commercialModeBadge[cp.commercial.mode] : 'Su preventivo'}
                    </span>
                    <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-carbone/38">
                      Arredo Chef selection
                    </span>
                  </div>

                  <h1 className="h-display mt-6 text-[clamp(2.15rem,5.4vw,4.5rem)] leading-[0.95] text-carbone">
                    {product.name}
                  </h1>
                  ...
                  {specs.length > 0 && (
                    <div className="mt-7 overflow-hidden rounded-[22px] border border-carbone/10 bg-white/75">
                      <div className="flex items-center justify-between gap-4 border-b border-carbone/10 px-5 py-4">
                        <p className="text-[9px] font-bold uppercase tracking-[0.23em] text-carbone/45">
                          Specifiche essenziali
                        </p>
                        <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-rosso">
                          Dati disponibili
                        </span>
                      </div>
                      <dl className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">
                        {specs.map((s, i) => (
                          <div
                            key={s.label}
                            className={`min-h-[92px] border-carbone/[0.08] p-4 ${
                              i % 2 ? 'border-l' : ''
                            } sm:border-l sm:first:border-l-0 lg:[&:nth-child(odd)]:border-l-0 xl:[&:nth-child(odd)]:border-l`}
                          >
                            <dt className="text-[9px] font-bold uppercase tracking-[0.17em] text-carbone/42">
                              {s.label}
                            </dt>
                            <dd className="mt-2 text-[13px] font-extrabold tabular-nums text-carbone sm:text-[14px]">
                              {s.value}
                            </dd>
                          </div>
                        ))}
                ...
                <Reveal delay={0.14}>
                  <div className="mt-8 rounded-[22px] border border-rosso/20 bg-[linear-gradient(135deg,rgba(216,35,42,.08),rgba(255,255,255,.55))] p-5 sm:p-6">
                    <p className="text-[9px] font-bold uppercase tracking-[0.22em] text-rosso">
                      Preventivo configurato
                    </p>
                    <p className="mt-3 text-sm leading-relaxed text-carbone/68">
                      Prezzo, trasporto e installazione vengono definiti sulla configurazione reale del tuo locale.
                    </p>
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      <a href={`tel:${company.phones.marketing.tel}`} className="btn-rosso group w-full">
                        <Phone size={15} />
                        Chiama Arredo Chef
                      </a>
                      <a
                    ...
                    <div className="mt-3">
                      <AddToProjectButton slug={product.slug} name={product.name} variant="cockpit" />
                    </div>
                  </div>
                </Reveal>
            ...
            <section className="mt-16 sm:mt-20 lg:mt-24">
              <Reveal>
                <div className="flex flex-wrap items-end justify-between gap-5">
                  <div>
                    <span className="text-[9px] font-bold uppercase tracking-[0.23em] text-rosso">
                      Arredo Chef · Stesso reparto
                    </span>
                    <h2 className="h-display mt-4 text-[clamp(2rem,4vw,3.3rem)] text-carbone">
                      Altre macchine da confrontare
                    </h2>
                  </div>
                  <Link
                    href={`/catalogo?categoria=${encodeURIComponent(product.category)}`}
                    className="group inline-flex items-center gap-2 text-sm font-bold text-carbone transition-colors hover:text-rosso"
                  >
                    Tutto il reparto
          ...
          <div className="mt-12 sm:mt-16">
            <Link
              href="/catalogo"
              className="group inline-flex items-center gap-2 text-sm font-bold text-carbone/60 transition-colors hover:text-rosso"
            >
