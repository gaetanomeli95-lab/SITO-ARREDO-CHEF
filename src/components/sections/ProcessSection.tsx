...
export default function ProcessSection() {
  ...
  const activeStep = process[active] ?? process[0];
  const detail = stepDetails[activeStep.n];
      ...
      <div className="container-ac relative">
            ...
            return (
              <Reveal key={step.n} delay={i * 0.05}>
                <article className={`overflow-hidden rounded-[22px] border transition-all duration-300 ${isActive ? 'border-rosso/30 bg-white shadow-[0_28px_65px_-42px_rgba(11,13,16,.5)]' : 'border-carbone/10 bg-white/65'}`}>
                  <button type="button" onClick={() => setActive(i)} aria-expanded={isActive} className="flex w-full items-center gap-4 p-5 text-left">
        ...
        <div className="mt-14 hidden lg:block">
          <div className="relative">
            ...
            <motion.div className="absolute left-[8%] top-7 h-px origin-left bg-rosso" animate={{ width: `${(active / Math.max(process.length - 1, 1)) * 84}%` }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }} />
            <div className="relative grid grid-cols-4 gap-4">
              {process.map((step, i) => {
                const isActive = active === i;
                return (
                  <button key={step.n} type="button" onClick={() => setActive(i)} aria-pressed={isActive} className="group text-left">
