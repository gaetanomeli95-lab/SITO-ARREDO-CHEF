...
export default function FeaturedProducts() {
  ...
  return (
    <section
      data-nav-theme="dark"
      className="relative overflow-hidden bg-carbone py-16 text-avorio sm:py-20 md:py-28 lg:py-36"
    >
      ...
      <div className="container-ac relative">
        <div className="grid gap-6 sm:gap-8 lg:grid-cols-[1fr_340px] lg:items-end">
          <Reveal>
            <div className="flex items-center gap-3 sm:gap-4">
              <span className="flex h-9 w-9 items-center justify-center rounded-full border border-rosso/35 bg-rosso/10 font-display text-[10px] font-extrabold text-rosso sm:h-10 sm:w-10 sm:text-[11px]">
                03
              </span>
          ...
          <Reveal delay={0.1}>
            <p className="border-l border-rosso/60 pl-5 text-pretty text-[13px] leading-relaxed text-avorio/72 sm:pl-6 sm:text-sm md:text-base">
          ...
          <div className="relative mt-8 overflow-hidden rounded-[24px] border border-white/12 bg-[#08090b] shadow-[0_45px_120px_-55px_rgba(0,0,0,.95)] sm:mt-12 sm:rounded-[30px] lg:min-h-[650px]">
            ...
            <div className="grid lg:min-h-[650px] lg:grid-cols-[190px_minmax(0,1fr)_370px]">
              <div className="relative z-20 flex gap-2 overflow-x-auto border-b border-white/10 bg-carbone/80 p-2.5 backdrop-blur-xl sm:p-3 lg:flex-col lg:overflow-visible lg:border-b-0 lg:border-r lg:p-4">
                <div className="mb-2 hidden items-center gap-2 px-2 pb-3 text-[8px] font-bold uppercase tracking-[0.3em] text-white/42 lg:flex">
                ...
                {featured.map((product, index) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={active}
                      className={`group relative flex min-w-[128px] items-center gap-2.5 overflow-hidden rounded-2xl border px-2.5 py-2.5 text-left transition-all duration-300 sm:min-w-[150px] sm:gap-3 sm:px-3 sm:py-3 lg:min-w-0 ${
                        active
                          ? 'border-rosso/55 bg-rosso/[0.10] text-white'
                          : 'border-white/[0.08] bg-white/[0.03] text-white/58 hover:border-white/18 hover:text-white/88'
                      }`}
                    >
                      ...
                      <span className="min-w-0">
                        <span className="block text-[7px] font-bold uppercase tracking-[0.18em] text-rosso sm:text-[8px] sm:tracking-[0.22em]">
                          M-{String(index + 1).padStart(2, '0')}
                        </span>
                        <span className="mt-1 block line-clamp-2 text-[9px] font-semibold leading-snug sm:text-[10px]">
                          {product.name}
                        </span>
                      </span>
              ...
              <div className="relative min-h-[340px] overflow-hidden sm:min-h-[420px] lg:min-h-0">
                ...
                <div className="absolute left-4 top-4 z-10 flex items-center gap-2 rounded-full border border-white/12 bg-black/38 px-3 py-2 text-[7px] font-bold uppercase tracking-[0.2em] text-white/62 backdrop-blur-md sm:left-5 sm:top-5 sm:text-[8px] sm:tracking-[0.25em]">
                ...
                <AnimatePresence mode="wait">
                  <motion.div
                    key={selected.slug}
                    initial={{ opacity: 0, y: 20, scale: 0.94, filter: 'blur(7px)' }}
                    animate={{ opacity: 1, y: 0, scale: 1, filter: 'blur(0px)' }}
                    exit={{ opacity: 0, y: -10, scale: 1.03, filter: 'blur(5px)' }}
                    transition={{ duration: 0.44, ease: [0.16, 1, 0.3, 1] }}
                    className="absolute inset-[15%_7%_5%] sm:inset-[13%_8%_5%]"
                  >
                    <Image
                      src={selected.image}
                      alt={selected.name}
                      ...
                      sizes="(max-width: 1023px) 86vw, 45vw"
                      className="object-contain drop-shadow-[0_34px_28px_rgba(0,0,0,.72)]"
                    />
                  </motion.div>
                </AnimatePresence>
              ...
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="relative z-20 flex flex-col border-t border-white/10 bg-carbone/92 p-5 backdrop-blur-2xl sm:p-6 lg:border-l lg:border-t-0 lg:p-8"
                >
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-[8px] font-bold uppercase tracking-[0.24em] text-rosso">
                      Machine profile
                    </span>
                    <Gauge size={15} className="text-white/35" />
                  </div>

                  <p className="mt-5 text-[9px] font-semibold uppercase tracking-[0.2em] text-white/48 sm:mt-8 sm:tracking-[0.23em]">
                    {selected.category}
                  </p>
                  <h3 className="mt-2 font-display text-[clamp(1.55rem,7vw,2.25rem)] font-extrabold leading-[1.02] tracking-tight text-white sm:mt-3 sm:text-[clamp(1.65rem,2.6vw,2.5rem)]">
                    {selected.name}
                  </h3>
                  ...
                  {specs.length > 0 ? (
                    <dl className="mt-5 border-t border-white/12 sm:mt-8">
                      {specs.map((spec, i) => (
                        <div
                          key={spec.label}
                          className={`grid grid-cols-[1fr_auto] gap-4 border-b border-white/10 py-2.5 text-[11px] sm:py-3 ${
                            i >= 3 ? 'hidden sm:grid' : ''
                          }`}
                        >
                          <dt className="text-white/52">{spec.label}</dt>
                          <dd className="text-right font-semibold tabular-nums text-white/88">
                            {spec.value}
                          </dd>
                        </div>
                      ))}
                    ...
                    <p className="mt-5 line-clamp-4 text-[13px] leading-relaxed text-white/62 sm:mt-7 sm:text-sm">
                  ...
                  <div className="mt-6 grid gap-3 sm:mt-8">
                    ...
                    <Link
                      ...
                      className="group flex w-full items-center justify-between rounded-full border border-white/14 px-6 py-3.5 text-sm font-semibold text-white/82 transition-all hover:border-rosso/50 hover:text-white sm:py-4"
                    >
                      Scheda completa
                      <ArrowUpRight
                        size={15}
        ...
        <div className="mt-6 flex flex-col items-start justify-between gap-4 sm:mt-8 sm:flex-row sm:items-center sm:gap-5">
          <p className="text-[8px] font-bold uppercase tracking-[0.22em] text-white/42 sm:text-[9px] sm:tracking-[0.3em]">
            {featured.length} macchine in primo piano · {products.length} nel catalogo
          </p>
          <Link href="/catalogo" className="group inline-flex items-center gap-3 text-sm font-bold text-white hover:text-rosso">
            ...
            <span className="flex h-10 w-10 items-center justify-center rounded-full border border-white/16 transition-all group-hover:border-rosso/50 group-hover:bg-rosso/10">
              <ArrowRight size={15} />
            </span>
          </Link>
        </div>
